...
const scrapeData = (year, measureType) => {
  const url = `https://www.capitol.hawaii.gov/advreports/advreport.aspx?year=
  ${year}&report=deadline&active=true&rpt_type=&measuretype=${measureType}`;
  const data = [];
  ...
  axios(url)
    ...
    }).catch(() => console.log('Failed, invalid input'));
  return data;
};
